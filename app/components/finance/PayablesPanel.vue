...
import { paySupplierInvoice } from '~/data'
...
/** Tab "AP Aging" — Menu Finance & ACC > Hutang & Opex (Penyederhanaan 7-Role/Menu). Dulu
 * `/finance/payables`, kini tab dalam satu menu bersama Opex — logika tidak diubah, ditambah aksi "Bayar"
 * (Fase 3.3 — Poros Project Order + Jurnal Finance) untuk tagihan berstatus Disetujui. */
...
function handlePay (supplierInvoiceId: string) {
  const invoice = paySupplierInvoice(supplierInvoiceId, currentUser.value.id)
  if (invoice) {
    showToast('Tagihan Dibayar', `${invoice.id} ditandai lunas — jurnal Dr Hutang Usaha/Cr Kas & Bank tercatat otomatis.`, 'success')
  } else {
    showToast('Gagal Membayar', 'Tagihan ini bukan berstatus Disetujui.', 'error')
  }
}

/**
 * Hutang vendor (revisi.md #1 — "tambah payable"). Diturunkan dari `SupplierInvoice` yang sudah ada di
 * modul Vendor & Partner, sehingga Finance dan Vendor & Partner selalu melihat angka yang sama.
 */
