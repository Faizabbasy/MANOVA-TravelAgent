...
/**
 * Tab "Credit & Debit Note" — Menu Finance & ACC > Invoice & Piutang (Penyederhanaan 7-Role/Menu). Dulu
 * `/finance/notes`, kini tab dalam satu menu bersama Invoice/AR Aging — logika tidak diubah.
 *
 * Sub-tab internal (Credit/Debit) memakai query param `ntab` — BUKAN `tab` seperti sebelumnya — karena
 * `tab` kini dipakai container `/finance/invoices` untuk memilih tab level-atas (Invoice/AR Aging/Credit
 * & Debit Note). Dua sistem tab bersarang tidak boleh berbagi key.
 */
...
const activeTab = computed<NotesTab>({
  get: () => (route.query.ntab === 'debit' ? 'debit' : 'credit'),
  set: value => router.replace({ query: { ...route.query, ntab: value } })
})
