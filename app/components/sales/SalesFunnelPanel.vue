...
import { Building2, FolderKanban } from 'lucide-vue-next'
import {
...
import { LEAD_STAGES, QUOTATION_APPROVAL_STATUSES } from '~/constants/status'
import { formatPercentage } from '~/utils/format'
import type { StatusBreakdownItem } from '~/components/shared/StatusBreakdownList.vue'

/**
 * Tab "Funnel" — Menu Sales > Pipeline (funnel Lead→Qualified→Quotation→Approved→Won→Client→Project Order).
 * Route lama tetap ada sebagai redirect (`app/pages/customer-journey/index.vue`). Tahap "Opportunity"
 * DIHAPUS (entitas Opportunity dihapus — lihat komentar desain di `app/types/lead.ts`), digantikan tahap
 * "Quotation" (Lead dengan `quotationId` terisi). Rekap Sumber Lead dipisah ke tab sendiri, lihat
 * `SalesLeadSourceRecapPanel.vue`.
 */
