...
import { Search, CheckCheck, Bell, ClipboardCheck, FolderKanban, Users, CalendarClock, FileText, Receipt, PlaneTakeoff, LifeBuoy } from 'lucide-vue-next'
...
/**
 * Tab "Notifications" — Menu Client Portal > Dashboard (Penyederhanaan 7-Role/Menu). Dulu
 * `/client/notifications`, kini tab dalam satu menu bersama Overview — logika tidak diubah. REUSE penuh
 * entitas `Notification`/`getNotificationsForUser`/`getUnreadNotificationCount`/`markNotificationRead`/
 * `markAllNotificationsRead` (Section 21, LOCKED shape), TIDAK membuat entitas notifikasi kedua. Isolasi
 * otomatis lewat `userId === currentUser.id` (pola sama `NotificationPanel.vue`) — tidak perlu filter
 * `clientScopeId` tambahan karena `Notification` sudah per-user, bukan per-company.
 */
