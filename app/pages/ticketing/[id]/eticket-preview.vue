    ...
    <div class="mx-auto max-w-3xl px-4 print:px-0 print:max-w-none">
      <template v-if="!booking">
        <div class="rounded-xl border border-border bg-card p-8 print:hidden">
          ...
          >
            <NuxtLink to="/ticketing">
              <Button>Kembali ke Ticketing</Button>
            </NuxtLink>
          </EmptyState>
        </div>
      </template>
      ...
      <template v-else>
        <div class="mb-4 flex items-center justify-between print:hidden">
          <NuxtLink :to="`/ticketing/${booking.id}`" class="text-sm text-primary hover:underline">
            ← Kembali ke Flight Booking
          </NuxtLink>
        ...
        <div class="rounded-xl border border-border bg-card p-8 shadow-sm print:rounded-none print:border-0 print:shadow-none print:p-0">
          <div class="flex items-start justify-between border-b border-border pb-6 mb-6">
            <div>
              ...
              <p class="text-xs text-muted-foreground">
                Travel Agent B2B — E-Ticket / Itinerary Receipt (Mock)
              </p>
            </div>
          ...
          <div class="grid grid-cols-2 gap-6 mb-6">
            ...
            <div>
              ...
              <p class="text-sm text-foreground">
                {{ booking.ticketingDeadline ? formatDate(booking.ticketingDeadline) : '—' }}
              </p>
            </div>
          </div>
          ...
          <table class="w-full text-sm mb-6">
            ...
            <tbody>
              ...
              <tr v-if="booking.segments.length === 0">
          ...
          <div v-if="selectedOption" class="grid grid-cols-2 gap-6 mb-6 text-sm">
            <div>
              ...
              <p class="text-foreground">
                {{ selectedOption.airline }} — {{ findStatusOption(CABIN_CLASSES, selectedOption.cabinClass).label }}
              </p>
              <p v-if="selectedOption.baggageAllowance" class="text-muted-foreground">
                Bagasi: {{ selectedOption.baggageAllowance }}
              </p>
              <p v-if="selectedOption.ancillaries" class="text-muted-foreground">
                {{ selectedOption.ancillaries }}
              </p>
            </div>
            <div class="text-right">
              <p class="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-1">
