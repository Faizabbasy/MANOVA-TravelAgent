...
function toggleVendorSelection(vendorId: string, checked: boolean) {
  if (checked) selectedVendorIds.value.push(vendorId)
  else selectedVendorIds.value = selectedVendorIds.value.filter(id => id !== vendorId)
}
function submitSend() {
    ...
    <template v-if="!rfq">
      ...
      <SectionCard>
        <EmptyState :icon="FileX" title="RFQ tidak ditemukan" :description="`RFQ dengan ID '${route.params.id}' tidak ada di data demo saat ini.`">
          <Button @click="router.push('/procurement')">Kembali ke Daftar Procurement</Button>
        </EmptyState>
      </SectionCard>
    </template>
    ...
    <template v-else>
        ...
        <template #actions>
          <div class="flex flex-wrap items-center gap-2">
            ...
            <template v-if="canManageProcurement">
              <Button v-if="rfq.status === 'draft'" size="sm" variant="outline" @click="openSendDialog"><Send class="h-4 w-4 mr-1.5" />Kirim ke Vendor</Button>
              <Button
                v-for="next in getRfqStatusTransitions(rfq.status)" :key="next"
                size="sm" variant="outline"
                @click="requestStatusChange(next)"
              >{{ findStatusOption(RFQ_STATUSES, next).label }}</Button>
            </template>
          </div>
        </template>
      </PageHeader>
      ...
      <SectionCard title="Line Items" description="Kebutuhan yang dikirim ke seluruh vendor yang diundang.">
        <Table>
          ...
          <TableBody>
            ...
            <TableEmpty v-if="rfq.lineItems.length === 0" :colspan="3">Belum ada line item.</TableEmpty>
          </TableBody>
        </Table>
      </SectionCard>
        ...
        <Table>
          ...
          <TableBody>
            <TableRow v-for="invitation in invitations" :key="invitation.id">
              <TableCell class="font-medium text-foreground">{{ vendorName(invitation.vendorId) }}</TableCell>
              <TableCell class="text-muted-foreground">{{ formatDate(invitation.invitedAt) }}</TableCell>
            ...
            <TableEmpty v-if="invitations.length === 0" :colspan="3">RFQ belum dikirim ke vendor manapun.</TableEmpty>
          </TableBody>
        </Table>
      </SectionCard>
          ...
          <Table>
            ...
            <TableBody>
              <TableRow v-for="response in responses" :key="response.id">
                <TableCell class="font-medium text-foreground">{{ vendorName(response.vendorId) }}</TableCell>
                <TableCell class="text-foreground">{{ formatCurrencyIdr(response.totalAmountIdr) }}</TableCell>
                <TableCell class="text-muted-foreground">{{ response.notes ?? '—' }}</TableCell>
                ...
                <TableCell v-if="canManageProcurement">
                  <Button
                    ...
                    size="sm" variant="outline" @click="openSelectDialog(response.vendorId)"
                  >Select</Button>
                </TableCell>
              </TableRow>
              <TableEmpty v-if="responses.length === 0" :colspan="canManageProcurement ? 6 : 5">Belum ada respons vendor.</TableEmpty>
            </TableBody>
          </Table>
        </div>
      </SectionCard>
      ...
      <SectionCard v-if="rfq.status === 'selected' || rfq.status === 'closed'" title="Service Order" description="Handoff formal dari RFQ ke Service Order setelah vendor terpilih.">
        <template v-if="canManageProcurement && relatedServiceOrders.length === 0" #actions>
          <Button size="sm" variant="outline" @click="createServiceOrderFromRfq">Buat Service Order</Button>
        </template>
        <ul v-if="relatedServiceOrders.length" class="divide-y divide-border">
          <li v-for="so in relatedServiceOrders" :key="so.id" class="py-2">
            <NuxtLink :to="`/procurement/service-orders/${so.id}`" class="text-sm font-medium text-primary hover:underline">{{ so.id }} — {{ vendorName(so.vendorId) }}</NuxtLink>
          </li>
        </ul>
      ...
      <SectionCard title="Clarification Thread" description="Thread klarifikasi per vendor.">
        <template #actions>
          <select v-if="invitations.length" v-model="clarificationVendorId" class="appearance-none px-3 py-2 text-sm rounded-lg border border-input bg-card text-foreground focus:outline-none focus:ring-2 focus:ring-ring cursor-pointer">
            <option v-for="invitation in invitations" :key="invitation.vendorId" :value="invitation.vendorId">{{ vendorName(invitation.vendorId) }}</option>
          </select>
        </template>
        ...
        <div v-if="canManageProcurement && clarificationVendorId" class="flex items-center gap-2 mt-4">
          ...
          <Button :disabled="!newClarificationMessage.trim()" @click="submitClarification"><Plus class="h-4 w-4 mr-1.5" />Kirim</Button>
        </div>
      </SectionCard>
      ...
      <Dialog v-model:open="isSendOpen">
        <DialogContent class="max-w-md">
          ...
          <div class="space-y-2 py-2 max-h-64 overflow-y-auto">
            ...
            <p v-if="candidateVendors.length === 0" class="text-xs text-muted-foreground">Tidak ada vendor dengan jenis layanan yang sesuai.</p>
          </div>
          <DialogFooter>
            <Button variant="outline" @click="isSendOpen = false">Batal</Button>
            <Button :disabled="selectedVendorIds.length === 0" @click="submitSend">Kirim</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog v-model:open="isSelectOpen">
        <DialogContent class="max-w-md">
          ...
          <DialogFooter>
            <Button variant="outline" @click="isSelectOpen = false">Batal</Button>
            <Button @click="confirmSelect">Konfirmasi Pilih</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </template>
