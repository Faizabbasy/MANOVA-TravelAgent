  ...
  <div class="space-y-6">
    <template v-if="!serviceOrder">
      ...
      <SectionCard>
        <EmptyState :icon="FileX" title="Service Order tidak ditemukan" :description="`Service Order dengan ID '${route.params.id}' tidak ada di data demo saat ini.`">
          <Button @click="router.push('/procurement?tab=service-orders')">
    ...
    <template v-else>
        ...
        <template #actions>
          <div class="flex flex-wrap items-center gap-2">
            ...
            <template v-if="canManageProcurement">
              <Button v-if="['acknowledged', 'amended'].includes(serviceOrder.status)" size="sm" variant="outline" @click="isAmendOpen = true">
                Amend
              </Button>
              <Button
                v-for="next in getServiceOrderStatusTransitions(serviceOrder.status)"
                :key="next"
                size="sm"
                :variant="next === 'cancelled' ? 'destructive' : 'outline'"
                @click="requestStatusChange(next)"
              >
                {{ findStatusOption(SERVICE_ORDER_STATUSES, next).label }}
              </Button>
            </template>
          </div>
        </template>
      ...
      <SectionCard title="Fulfillment Status Timeline">
        <div v-if="serviceOrder.status === 'cancelled'" class="text-sm text-destructive font-medium">
          Service Order dibatalkan{{ serviceOrder.statusReason ? ` — Alasan: ${serviceOrder.statusReason}` : '' }}.
        </div>
        <ol v-else class="flex flex-wrap items-center gap-2">
          ...
          <li v-if="serviceOrder.status === 'amended'" class="text-xs font-medium px-2.5 py-1 rounded-full border border-purple-500 text-purple-600">
            Amended (di luar jalur utama)
          </li>
        </ol>
      </SectionCard>
        ...
        <div class="grid gap-3 sm:grid-cols-2 mt-4">
          <div v-if="canViewProcurementFinancials" class="rounded-lg border border-border p-3">
            ...
            <p class="text-lg font-semibold text-foreground">
              {{ serviceOrder.netCostIdr !== undefined ? formatCurrencyIdr(serviceOrder.netCostIdr) : '—' }}
            </p>
          </div>
          <div class="rounded-lg border border-border p-3">
            ...
            <p class="text-lg font-semibold text-foreground">
              {{ serviceOrder.sellPriceIdr !== undefined ? formatCurrencyIdr(serviceOrder.sellPriceIdr) : '—' }}
            </p>
          </div>
        </div>
        <p v-if="!canViewProcurementFinancials" class="mt-2 text-xs text-muted-foreground">
