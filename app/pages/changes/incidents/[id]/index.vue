  ...
  <div class="space-y-6">
    <template v-if="!incident">
      ...
      <SectionCard>
        <EmptyState :icon="FileX" title="Incident tidak ditemukan" :description="`Incident dengan ID '${route.params.id}' tidak ada di data demo saat ini.`">
          <Button @click="router.push('/changes?tab=incidents')">Kembali ke Changes & Incidents</Button>
        </EmptyState>
      </SectionCard>
    </template>
    ...
    <template v-else>
        ...
        <template #actions>
          <div class="flex flex-wrap items-center gap-2">
            ...
            <template v-if="canManageChanges">
              <Button v-if="getIncidentStatusTransitions(incident.status).includes('investigating')" size="sm" variant="outline" @click="handleTransition('investigating')">Mulai Investigasi</Button>
              <Button v-if="getIncidentStatusTransitions(incident.status).includes('escalated')" size="sm" variant="destructive" @click="openEscalate">Eskalasi</Button>
              <Button v-if="getIncidentStatusTransitions(incident.status).includes('resolved')" size="sm" @click="openResolve">Selesaikan</Button>
              <Button v-if="getIncidentStatusTransitions(incident.status).includes('closed')" size="sm" variant="outline" @click="handleClose">Tutup</Button>
            </template>
          </div>
        </template>
      </PageHeader>
      ...
      <SectionCard title="Deskripsi">
        <p class="text-sm text-foreground whitespace-pre-line">{{ incident.description }}</p>
      </SectionCard>

      <SectionCard v-if="incident.resolutionNote" title="Resolusi">
        <p class="text-sm text-foreground whitespace-pre-line">{{ incident.resolutionNote }}</p>
        <p v-if="incident.resolvedAt" class="text-xs text-muted-foreground mt-1">Diselesaikan: {{ formatDate(incident.resolvedAt) }}</p>
      </SectionCard>
      ...
      <SectionCard title="Communication Log">
        <ul v-if="incident.communicationLog.length" class="space-y-3 mb-4">
          <li v-for="entry in incident.communicationLog" :key="entry.id" class="rounded-lg border border-border p-3">
            <div class="flex items-center justify-between gap-3 mb-1">
              <p class="text-sm font-medium text-foreground">{{ getUserById(entry.from)?.name ?? entry.from }}</p>
              <p class="text-xs text-muted-foreground">{{ formatDateTime(entry.at) }}</p>
            </div>
        ...
        <div v-if="canManageChanges" class="flex items-center gap-2 pt-2 border-t border-border mt-4">
          ...
          <Button size="sm" :disabled="!communicationMessage.trim()" @click="submitCommunication"><Send class="h-4 w-4 mr-1.5" />Kirim</Button>
        </div>
      </SectionCard>
      ...
      <Dialog v-model:open="isEscalateOpen">
        <DialogContent class="max-w-md">
          ...
          <div class="space-y-4 py-2">
            <div class="space-y-1.5">
              ...
              <select id="escalate-to" v-model="escalateTo" class="w-full appearance-none px-3 py-2 text-sm rounded-lg border border-input bg-card text-foreground focus:outline-none focus:ring-2 focus:ring-ring cursor-pointer">
                <option value="" disabled>Pilih penerima eskalasi</option>
                <option v-for="user in escalationCandidates" :key="user.id" :value="user.id">{{ user.name }} ({{ user.role }})</option>
              </select>
            </div>
            <div class="space-y-1.5">
          ...
          <DialogFooter>
            <Button variant="outline" @click="isEscalateOpen = false">Batal</Button>
            <Button variant="destructive" :disabled="!escalateTo" @click="submitEscalate">Eskalasi</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog v-model:open="isResolveOpen">
        <DialogContent class="max-w-md">
          ...
          <DialogFooter>
            <Button variant="outline" @click="isResolveOpen = false">Batal</Button>
            <Button :disabled="!resolutionNote.trim()" @click="submitResolve">Selesaikan</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </template>
