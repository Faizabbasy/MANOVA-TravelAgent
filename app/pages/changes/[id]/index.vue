...
import type { ChangeRequestStatus } from '~/types/change-incident'
    ...
    <template v-if="!request">
      ...
      <SectionCard>
        <EmptyState :icon="FileX" title="Change Request tidak ditemukan" :description="`Change Request dengan ID '${route.params.id}' tidak ada di data demo saat ini.`">
          <Button @click="router.push('/changes')">Kembali ke Changes & Incidents</Button>
        </EmptyState>
      </SectionCard>
    </template>
    ...
    <template v-else>
        ...
        <template #actions>
          <div class="flex flex-wrap items-center gap-2">
            ...
            <template v-if="canApproveChangeRequest">
              <Button v-if="getChangeRequestStatusTransitions(request.status).includes('under-review')" size="sm" variant="outline" @click="handleUnderReview">Mulai Review</Button>
              <Button v-if="getChangeRequestStatusTransitions(request.status).includes('approved')" size="sm" @click="handleApprove">Setujui</Button>
              <Button v-if="getChangeRequestStatusTransitions(request.status).includes('rejected')" size="sm" variant="destructive" @click="openReject">Tolak</Button>
              <Button v-if="getChangeRequestStatusTransitions(request.status).includes('implemented')" size="sm" variant="outline" @click="handleImplemented">Tandai Diimplementasikan</Button>
            </template>
          </div>
        </template>
      </PageHeader>
      ...
      <SectionCard title="Before / After">
        <div class="grid gap-4 sm:grid-cols-2">
          <div class="rounded-lg border border-border p-3">
            <p class="text-xs text-muted-foreground">Sebelum</p>
            <p class="text-sm text-foreground mt-1">{{ request.beforeSummary }}</p>
          </div>
          <div class="rounded-lg border border-border p-3">
            <p class="text-xs text-muted-foreground">Sesudah</p>
            <p class="text-sm text-foreground mt-1">{{ request.afterSummary }}</p>
          </div>
        </div>
      </SectionCard>
      ...
      <SectionCard title="Dampak">
        <div class="grid gap-3 sm:grid-cols-2">
          <div>
            <p class="text-xs text-muted-foreground">Dampak Operasional</p>
            <p class="text-sm text-foreground mt-1">{{ request.operationalImpact ?? '—' }}</p>
          </div>
          <div>
            <p class="text-xs text-muted-foreground">Dampak Komersial</p>
            <p class="text-sm text-foreground mt-1">{{ request.commercialImpactIdr !== undefined ? formatCurrencyIdr(request.commercialImpactIdr) : '—' }}</p>
          </div>
          <div>
            <p class="text-xs text-muted-foreground">Dampak Finansial (Catatan)</p>
            <p class="text-sm text-foreground mt-1">{{ request.financialImpactNote ?? '—' }}</p>
          </div>
          <div>
            <p class="text-xs text-muted-foreground">Dampak Timeline</p>
            <p class="text-sm text-foreground mt-1">{{ request.timelineImpactNote ?? '—' }}</p>
          </div>
        </div>
      </SectionCard>

      <SectionCard title="Additional Quotation / Change Order">
        <p v-if="linkedQuotation" class="text-sm text-foreground">{{ linkedQuotation.id }} — {{ formatCurrencyIdr(linkedQuotation.amountIdr) }}</p>
        <p v-else class="text-sm text-muted-foreground">Tidak ada quotation tambahan/change order yang ditautkan pada Change Request ini.</p>
      </SectionCard>

      <SectionCard title="Approval / Rejection">
        <div class="grid gap-3 sm:grid-cols-2 text-sm">
          <p class="text-muted-foreground">Disetujui/Ditolak Oleh: <span class="text-foreground">{{ request.approvedBy ? (getUserById(request.approvedBy)?.name ?? request.approvedBy) : '—' }}</span></p>
          <p class="text-muted-foreground">Tanggal: <span class="text-foreground">{{ request.approvedAt ? formatDate(request.approvedAt) : '—' }}</span></p>
        </div>
        <p v-if="request.rejectionReason" class="text-sm text-destructive mt-2">Alasan Penolakan: {{ request.rejectionReason }}</p>
        <p v-if="request.activityEntryId" class="text-xs text-muted-foreground mt-3">
          Tertaut ke Activity & Changes: <NuxtLink :to="`/projects/${request.projectId}?tab=activity-changes`" class="text-primary hover:underline">{{ request.activityEntryId }}</NuxtLink>
        </p>
      </SectionCard>
