    ...
    <div class="mx-auto max-w-3xl px-4 print:px-0 print:max-w-none">
      <template v-if="!booking">
        <div class="rounded-xl border border-border bg-card p-8 print:hidden">
          ...
          >
            <NuxtLink to="/accommodation"><Button>Kembali ke Accommodation</Button></NuxtLink>
          </EmptyState>
        </div>
      </template>
      ...
      <template v-else>
        <div class="mb-4 flex items-center justify-between print:hidden">
          <NuxtLink :to="`/accommodation/${booking.id}`" class="text-sm text-primary hover:underline">← Kembali ke Hotel Booking</NuxtLink>
        ...
        <div class="rounded-xl border border-border bg-card p-8 shadow-sm print:rounded-none print:border-0 print:shadow-none print:p-0">
          <div class="flex items-start justify-between border-b border-border pb-6 mb-6">
            <div>
              ...
              <p class="text-xs text-muted-foreground">Travel Agent B2B — Hotel Voucher (Mock)</p>
            </div>
            <div class="text-right">
              <p class="text-lg font-semibold text-foreground">VOUCHER</p>
              <p class="text-sm text-muted-foreground">Konfirmasi: {{ booking.confirmationNumber ?? 'Belum terbit' }}</p>
          ...
          <div class="grid grid-cols-2 gap-6 mb-6">
            ...
            <div>
              <p class="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-1">Check-in / Check-out</p>
              <p class="text-sm text-foreground">{{ booking.checkInDate ? formatDate(booking.checkInDate) : '—' }} – {{ booking.checkOutDate ? formatDate(booking.checkOutDate) : '—' }}</p>
          ...
          <div v-if="selectedOption" class="grid grid-cols-2 gap-6 mb-6 text-sm">
            <div>
              <p class="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-1">Property / Room Type</p>
              <p class="text-foreground">{{ selectedOption.propertyName }} — {{ selectedOption.roomType }}</p>
              <p class="text-muted-foreground">{{ selectedOption.ratePlan }} · {{ findStatusOption(MEAL_PLANS, selectedOption.mealPlan).label }}</p>
              <p v-if="selectedOption.policies" class="text-muted-foreground">{{ selectedOption.policies }}</p>
            </div>
            <div class="text-right">
              ...
              <p v-if="booking.roomsBlocked" class="text-xs text-muted-foreground">{{ booking.roomsBlocked }} kamar</p>
            </div>
          </div>

          <div class="mb-6">
            <p class="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-2">Traveler / Guest List</p>
          ...
          <div v-if="booking.cancellationDeadline" class="border-t border-border pt-4 text-xs text-muted-foreground">
            <p class="font-semibold uppercase tracking-wide mb-1">Cancellation Policy</p>
