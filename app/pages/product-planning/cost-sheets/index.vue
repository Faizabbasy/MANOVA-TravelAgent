      ...
      <template v-if="canManageCostSheet" #actions>
        <Dialog v-model:open="isCreateOpen">
          <DialogTrigger as-child>
            <Button @click="openCreateDialog"><Plus class="h-4 w-4 mr-1.5" />Buat Cost Sheet</Button>
          </DialogTrigger>
          <DialogScrollContent class="max-w-md">
            ...
            <div class="space-y-4 py-2">
              ...
              <div class="space-y-1.5">
                ...
                <select id="cs-product" v-model="newProductId" class="w-full appearance-none px-3 py-2 text-sm rounded-lg border border-input bg-card text-foreground focus:outline-none focus:ring-2 focus:ring-ring cursor-pointer">
                  ...
                  <option v-for="product in PRODUCT_TEMPLATES" :key="product.id" :value="product.id">{{ product.name }}</option>
                </select>
              </div>
              <div class="space-y-1.5">
                ...
                <select id="cs-opportunity" v-model="newOpportunityId" class="w-full appearance-none px-3 py-2 text-sm rounded-lg border border-input bg-card text-foreground focus:outline-none focus:ring-2 focus:ring-ring cursor-pointer">
                  <option value="">Belum terhubung ke Opportunity</option>
                  <option v-for="opp in opportunitiesWithCostSheetInterest" :key="opp.id" :value="opp.id">{{ opp.title }}</option>
                </select>
              </div>
              <div class="space-y-1.5">
            ...
            <DialogFooter>
              ...
              <Button :disabled="!newName.trim() || !newTravelerCount" @click="submitCreate">Simpan</Button>
            </DialogFooter>
          </DialogScrollContent>
        </Dialog>
      </template>
    ...
    <template v-else>
      <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        <select v-model="opportunityFilter" class="appearance-none px-3 py-2 text-sm rounded-lg border border-input bg-card text-foreground focus:outline-none focus:ring-2 focus:ring-ring cursor-pointer">
          <option value="all">Semua Opportunity</option>
          <option v-for="opp in OPPORTUNITIES" :key="opp.id" :value="opp.id">{{ opp.title }}</option>
        </select>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            <TableRow v-for="row in rows" :key="row.sheet.id" class="cursor-pointer hover:bg-muted/50" @click="navigateTo(`/product-planning/cost-sheets/${row.sheet.id}`)">
              <TableCell class="font-medium text-foreground">{{ row.sheet.name }}</TableCell>
              <TableCell class="text-muted-foreground">{{ opportunityLabel(row.sheet.opportunityId) }}</TableCell>
              <TableCell class="text-muted-foreground">{{ row.sheet.travelerCount }} pax</TableCell>
              <TableCell class="text-foreground">{{ formatCurrencyIdr(row.breakdown.totalSellIdr) }}</TableCell>
              <TableCell class="text-muted-foreground">v{{ row.sheet.version }}</TableCell>
              <TableCell>
