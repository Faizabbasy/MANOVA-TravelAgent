  ...
  <div class="space-y-6">
    <template v-if="!event">
      ...
      <SectionCard>
        ...
        >
          <Button @click="router.push('/mice')">Kembali ke Daftar MICE</Button>
        </EmptyState>
      </SectionCard>
    </template>
    ...
    <template v-else>
        ...
        <template #actions>
          <div class="flex flex-wrap items-center gap-2">
            ...
            <NuxtLink :to="`/mice/${event.id}/rundown-preview`" target="_blank">
              <Button size="sm" variant="outline"><Printer class="h-4 w-4 mr-1.5" />Rundown</Button>
            </NuxtLink>
            <NuxtLink :to="`/mice/${event.id}/boq-preview`" target="_blank">
              <Button size="sm" variant="outline"><ClipboardList class="h-4 w-4 mr-1.5" />BOQ</Button>
            </NuxtLink>
            <template v-if="canManageMice">
              <Button size="sm" variant="outline" @click="openEditDialog">Edit</Button>
              <Button
                v-for="next in getMiceEventStatusTransitions(event.status)" :key="next"
                size="sm" :variant="next === 'cancelled' ? 'destructive' : 'outline'"
                @click="requestStatusChange(next)"
              >{{ findStatusOption(MICE_EVENT_STATUSES, next).label }}</Button>
            </template>
          </div>
        </template>
      </PageHeader>
      ...
      <div v-if="conflicts.length" class="rounded-lg border border-destructive/40 bg-destructive/5 px-4 py-3 text-sm">
        <p class="font-semibold text-foreground flex items-center gap-1.5"><AlertTriangle class="h-4 w-4" />Capacity / Schedule Conflict</p>
        <ul class="text-muted-foreground mt-1 list-disc list-inside space-y-0.5">
          <li v-for="(conflict, index) in conflicts" :key="index">{{ conflict }}</li>
        </ul>
      </div>
      <div v-if="event.hasChangeOrder" class="rounded-lg border border-amber-500/40 bg-amber-500/5 px-4 py-3 text-sm">
        <p class="font-semibold text-foreground">Change Order</p>
        <p class="text-muted-foreground mt-1">{{ event.changeOrderNote || 'Terjadi perubahan pasca-approval — detail belum dicatat.' }}</p>
      </div>
      <div v-if="event.hasIncident" class="rounded-lg border border-purple-500/40 bg-purple-500/5 px-4 py-3 text-sm">
        ...
        <p class="text-muted-foreground mt-1">{{ event.incidentNote || 'Terjadi insiden operasional — detail belum dicatat.' }}</p>
      </div>

      <SectionCard>
        <p v-if="event.brief" class="text-sm text-foreground mb-4 whitespace-pre-line">{{ event.brief }}</p>
        <DetailMetadataList :items="summaryMetadata" />
        <template v-if="canManageMice" #actions>
          <div class="flex flex-wrap gap-2">
            <Button
              v-for="next in getMiceApprovalTransitions(event.clientApprovalStatus)" :key="next"
              size="sm" :variant="next === 'rejected' ? 'destructive' : 'outline'"
              @click="requestApprovalChange(next)"
            >{{ next === 'submitted' ? 'Ajukan ke Client' : findStatusOption(MICE_APPROVAL_STATUSES, next).label }}</Button>
          </div>
        </template>
      </SectionCard>
      ...
      <SectionCard title="Rooms / Sessions / Agenda" description="Venue per sesi, jadwal, kapasitas, dan PIC.">
        <template v-if="canManageMice" #actions>
          <Button size="sm" variant="outline" @click="openSessionsDialog">Kelola Sessions</Button>
        </template>
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            <TableRow v-for="(session, index) in event.sessions" :key="index">
              <TableCell class="text-foreground">{{ session.sessionTitle }}</TableCell>
              <TableCell class="text-muted-foreground">{{ session.roomName }}</TableCell>
              <TableCell class="text-muted-foreground">{{ formatDateTime(session.startAt) }} – {{ formatDateTime(session.endAt) }}</TableCell>
              <TableCell class="text-muted-foreground">{{ session.capacity }} pax</TableCell>
              <TableCell class="text-muted-foreground">{{ userName(session.picUserId) }}</TableCell>
              <TableCell><StatusBadge :label="session.isConfirmed ? 'Confirmed' : 'Tentatif'" :tone="session.isConfirmed ? 'success' : 'warning'" /></TableCell>
            </TableRow>
            <TableEmpty v-if="event.sessions.length === 0" :colspan="6">Belum ada sesi tercatat.</TableEmpty>
          </TableBody>
        </Table>
      </SectionCard>
      ...
      <SectionCard title="Participant Categories / Attendance" description="Kategori peserta, target, dan realisasi kehadiran.">
        <template v-if="canManageMice" #actions>
          <Button size="sm" variant="outline" @click="openParticipantsDialog">Kelola Peserta</Button>
        </template>
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            <TableRow v-for="(category, index) in event.participantCategories" :key="index">
              <TableCell class="text-foreground">{{ category.category }}</TableCell>
              <TableCell class="text-muted-foreground">{{ category.expectedCount }} pax</TableCell>
              <TableCell class="text-muted-foreground">{{ category.actualCount !== undefined ? `${category.actualCount} pax` : '—' }}</TableCell>
            </TableRow>
            <TableEmpty v-if="event.participantCategories.length === 0" :colspan="3">Belum ada kategori peserta tercatat.</TableEmpty>
          </TableBody>
        </Table>
      </SectionCard>
      ...
      <SectionCard title="BOQ (Bill of Quantities)" description="Catering, AV, staging, equipment, booth, dan vendor package.">
        <template v-if="canManageMice" #actions>
          <Button size="sm" variant="outline" @click="openBoqDialog">Kelola BOQ</Button>
        </template>
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead v-if="canViewMiceFinancials">Net Cost</TableHead>
          ...
          <TableBody>
            <TableRow v-for="(item, index) in event.boqItems" :key="index">
              ...
              <TableCell class="text-muted-foreground">{{ item.quantity }} {{ item.unit }}</TableCell>
              <TableCell class="text-muted-foreground">{{ vendorName(item.vendorId) }}</TableCell>
              <TableCell v-if="canViewMiceFinancials" class="text-foreground">{{ item.netCostIdr !== undefined ? formatCurrencyIdr(item.netCostIdr) : '—' }}</TableCell>
              <TableCell class="text-foreground">{{ item.sellPriceIdr !== undefined ? formatCurrencyIdr(item.sellPriceIdr) : '—' }}</TableCell>
            </TableRow>
            <TableEmpty v-if="event.boqItems.length === 0" :colspan="canViewMiceFinancials ? 6 : 5">Belum ada baris BOQ tercatat.</TableEmpty>
          </TableBody>
        </Table>
        <div class="grid gap-3 sm:grid-cols-3 mt-4">
          <div v-if="canViewMiceFinancials" class="rounded-lg border border-border p-3">
            <p class="text-xs text-muted-foreground">Total Net Cost (Internal)</p>
            <p class="text-lg font-semibold text-foreground">{{ totals.netCostIdr !== undefined ? formatCurrencyIdr(totals.netCostIdr) : '—' }}</p>
          </div>
          <div class="rounded-lg border border-border p-3">
            <p class="text-xs text-muted-foreground">Total Sell Price (Client)</p>
            <p class="text-lg font-semibold text-foreground">{{ totals.sellPriceIdr !== undefined ? formatCurrencyIdr(totals.sellPriceIdr) : '—' }}</p>
          </div>
          <div v-if="canViewMiceFinancials" class="rounded-lg border border-border p-3">
            <p class="text-xs text-muted-foreground">Margin</p>
            <p class="text-lg font-semibold text-foreground">{{ totals.marginIdr !== undefined ? formatCurrencyIdr(totals.marginIdr) : '—' }}</p>
          </div>
        </div>
        <p v-if="!canViewMiceFinancials" class="mt-2 text-xs text-muted-foreground">Net cost internal tidak ditampilkan untuk role ini.</p>
      </SectionCard>

      <SectionCard title="Staffing / PIC" description="Penanggung jawab internal per area kerja.">
        <template v-if="canManageMice" #actions>
          <Button size="sm" variant="outline" @click="openStaffingDialog">Kelola Staffing</Button>
        </template>
        ...
        <template v-if="canManageMice" #actions>
          <Dialog v-model:open="isChecklistAddOpen">
            <DialogTrigger as-child>
              <Button size="sm" variant="outline"><Plus class="h-4 w-4 mr-1.5" />Tambah Item</Button>
            </DialogTrigger>
            <DialogContent class="max-w-sm">
              ...
              <div class="space-y-3 py-2">
                <div class="space-y-1.5">
                  ...
                  <select id="checklist-task" v-model="newChecklistTask" class="w-full appearance-none px-3 py-2 text-sm rounded-lg border border-input bg-card text-foreground focus:outline-none focus:ring-2 focus:ring-ring cursor-pointer">
                    <option v-for="t in MICE_CHECKLIST_TASKS" :key="t.value" :value="t.value">{{ t.label }}</option>
                  </select>
                </div>
                <div class="space-y-1.5">
              ...
              <DialogFooter>
                <Button variant="outline" @click="isChecklistAddOpen = false">Batal</Button>
                <Button :disabled="!newChecklistLabel.trim()" @click="submitAddChecklist">Simpan</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </template>
      ...
      <SectionCard title="Deliverables">
        <template v-if="canManageMice" #actions>
          <Dialog v-model:open="isDeliverableAddOpen">
            <DialogTrigger as-child>
              <Button size="sm" variant="outline"><Plus class="h-4 w-4 mr-1.5" />Tambah Deliverable</Button>
            </DialogTrigger>
            <DialogContent class="max-w-sm">
              ...
              <DialogFooter>
                <Button variant="outline" @click="isDeliverableAddOpen = false">Batal</Button>
                <Button :disabled="!newDeliverableLabel.trim()" @click="submitAddDeliverable">Simpan</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </template>
      ...
      <Dialog v-model:open="isApprovalDialogOpen">
        <DialogContent class="max-w-md">
          ...
          <DialogFooter>
            <Button variant="outline" @click="isApprovalDialogOpen = false">Batal</Button>
            <Button variant="destructive" :disabled="!approvalNote.trim()" @click="submitApprovalChange">Konfirmasi</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogScrollContent class="max-w-2xl">
          ...
          <div class="space-y-2 py-2">
            <div class="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" type="button" @click="addSessionRow"><Plus class="h-3.5 w-3.5 mr-1" />Tambah</Button>
            </div>
            <div v-for="(session, index) in editSessions" :key="index" class="grid grid-cols-12 gap-2 items-center border-b border-border pb-2">
              ...
              <button type="button" class="col-span-1 text-muted-foreground hover:text-destructive" @click="removeSessionRow(index)"><Trash2 class="h-4 w-4" /></button>
              ...
              <select v-model="session.picUserId" class="col-span-3 appearance-none px-2 py-1.5 text-xs rounded-lg border border-input bg-card text-foreground focus:outline-none focus:ring-2 focus:ring-ring cursor-pointer">
                <option :value="undefined">PIC (opsional)</option>
            ...
            <p v-if="editSessions.length === 0" class="text-xs text-muted-foreground">Belum ada sesi — klik "Tambah".</p>
          </div>
          <DialogFooter>
            <Button variant="outline" @click="isSessionsOpen = false">Batal</Button>
            <Button @click="submitSessions">Simpan</Button>
          </DialogFooter>
        </DialogScrollContent>
      </Dialog>
        ...
        <DialogScrollContent class="max-w-lg">
          ...
          <div class="space-y-2 py-2">
            <div class="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" type="button" @click="addParticipantRow"><Plus class="h-3.5 w-3.5 mr-1" />Tambah</Button>
            </div>
            <div v-for="(category, index) in editParticipants" :key="index" class="grid grid-cols-12 gap-2 items-center">
              ...
              <button type="button" class="col-span-1 text-muted-foreground hover:text-destructive" @click="removeParticipantRow(index)"><Trash2 class="h-4 w-4" /></button>
            </div>
            <p v-if="editParticipants.length === 0" class="text-xs text-muted-foreground">Belum ada kategori — klik "Tambah".</p>
          </div>
          <DialogFooter>
            <Button variant="outline" @click="isParticipantsOpen = false">Batal</Button>
            <Button @click="submitParticipants">Simpan</Button>
          </DialogFooter>
        </DialogScrollContent>
      </Dialog>
        ...
        <DialogScrollContent class="max-w-2xl">
          ...
          <div class="space-y-2 py-2">
            <div class="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" type="button" @click="addBoqRow"><Plus class="h-3.5 w-3.5 mr-1" />Tambah</Button>
            </div>
            <div v-for="(item, index) in editBoq" :key="index" class="grid grid-cols-12 gap-2 items-center border-b border-border pb-2">
              <select v-model="item.category" class="col-span-2 appearance-none px-2 py-1.5 text-xs rounded-lg border border-input bg-card text-foreground focus:outline-none focus:ring-2 focus:ring-ring cursor-pointer">
                <option v-for="c in MICE_BOQ_CATEGORIES" :key="c.value" :value="c.value">{{ c.label }}</option>
              </select>
              ...
              <select v-model="item.vendorId" class="col-span-2 appearance-none px-2 py-1.5 text-xs rounded-lg border border-input bg-card text-foreground focus:outline-none focus:ring-2 focus:ring-ring cursor-pointer">
                <option :value="undefined">Vendor (opsional)</option>
                <option v-for="vendor in VENDORS" :key="vendor.id" :value="vendor.id">{{ vendor.name }}</option>
              </select>
              <button type="button" class="col-span-1 text-muted-foreground hover:text-destructive" @click="removeBoqRow(index)"><Trash2 class="h-4 w-4" /></button>
            ...
            <p v-if="editBoq.length === 0" class="text-xs text-muted-foreground">Belum ada baris BOQ — klik "Tambah".</p>
          </div>
          <DialogFooter>
            <Button variant="outline" @click="isBoqOpen = false">Batal</Button>
            <Button @click="submitBoq">Simpan</Button>
          </DialogFooter>
        </DialogScrollContent>
      </Dialog>
        ...
        <DialogScrollContent class="max-w-lg">
          ...
          <div class="space-y-2 py-2">
            <div class="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" type="button" @click="addStaffingRow"><Plus class="h-3.5 w-3.5 mr-1" />Tambah</Button>
            </div>
            <div v-for="(staff, index) in editStaffing" :key="index" class="grid grid-cols-12 gap-2 items-center">
              ...
              <button type="button" class="col-span-1 text-muted-foreground hover:text-destructive" @click="removeStaffingRow(index)"><Trash2 class="h-4 w-4" /></button>
            </div>
            <p v-if="editStaffing.length === 0" class="text-xs text-muted-foreground">Belum ada penugasan — klik "Tambah".</p>
          </div>
          <DialogFooter>
            <Button variant="outline" @click="isStaffingOpen = false">Batal</Button>
            <Button @click="submitStaffing">Simpan</Button>
          </DialogFooter>
        </DialogScrollContent>
      </Dialog>
    </template>
