        ...
        <div class="rounded-xl border border-border bg-card p-8 shadow-sm print:rounded-none print:border-0 print:shadow-none print:p-0">
          <div class="flex items-start justify-between border-b border-border pb-6 mb-6">
            <div>
              ...
              <p class="text-xs text-muted-foreground">Travel Agent B2B — Event Rundown (Mock)</p>
            </div>
            <div class="text-right">
              <p class="text-lg font-semibold text-foreground">RUNDOWN</p>
          ...
          <div class="grid grid-cols-2 gap-6 mb-6">
            ...
            <div>
              ...
              <p class="text-sm text-muted-foreground">{{ event.venueAddress }}</p>
            </div>
          </div>

          <div v-if="event.brief" class="mb-6 text-sm">
            <p class="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-1">Brief</p>
            <p class="text-foreground whitespace-pre-line">{{ event.brief }}</p>
          </div>
          ...
          <table class="w-full text-sm mb-6">
            ...
            <tbody>
              <tr v-for="(session, index) in event.sessions" :key="index" class="border-b border-border/60">
                <td class="py-2 text-foreground">{{ session.sessionTitle }}{{ !session.isConfirmed ? ' (tentatif)' : '' }}</td>
                <td class="py-2 text-muted-foreground">{{ session.roomName }}</td>
                <td class="py-2 text-muted-foreground">{{ formatDateTime(session.startAt) }} – {{ formatDateTime(session.endAt) }}</td>
                <td class="py-2 text-muted-foreground">{{ userName(session.picUserId) }}</td>
              </tr>
              <tr v-if="event.sessions.length === 0"><td colspan="4" class="py-2 text-muted-foreground">Belum ada sesi tercatat.</td></tr>
            </tbody>
          </table>

          <div class="mb-6">
            <p class="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-2">Participant Categories</p>
            <ul class="text-sm text-foreground grid grid-cols-2 gap-1">
              <li v-for="(category, index) in event.participantCategories" :key="index">{{ category.category }} — {{ category.expectedCount }} pax</li>
            </ul>
            <p v-if="event.participantCategories.length === 0" class="text-sm text-muted-foreground">Belum ada kategori peserta.</p>
          </div>
