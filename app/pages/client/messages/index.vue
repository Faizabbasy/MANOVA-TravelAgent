...
/**
 * Messages & Activities — Index (Repair Phase Section 6 — Finance & Collaboration, Master Prompt bagian
 * 13). Konsolidasi PER PROJECT (route `/client/messages/[projectId]`) — "Quotation conversation"/"Itinerary
 * conversation"/"Change request conversation" DIWUJUDKAN sebagai link ke comment thread yang SUDAH ADA
 * masing-masing (`QuotationComment`/`ItineraryComment`/`ChangeRequestComment`, Section 3/4/5) di halaman
 * detail penuh entitas tsb, BUKAN diimplementasikan ulang sebagai channel `Message` kedua untuk entitas yang
 * sama (satu source of truth per entitas, konsisten preseden section-section sebelumnya).
 */
definePageMeta({ layout: 'dashboard', middleware: 'auth' })
useHead({ title: 'Messages & Activities' })
...
<template>
  <div class="space-y-6">
    <PageHeader
      ...
      description="Percakapan dan timeline aktivitas per Project Order Anda."
      :breadcrumb="[{ label: 'Client Portal', to: '/client' }, { label: 'Collaboration' }, { label: 'Messages & Activities' }]"
    />
