...
const actionQueue = computed<AttentionItem[]>(() => {
  ...
  for (const row of quotationsAwaitingConfirm.value) {
    items.push({ id: `approval-quotation-${row.opportunity.id}`, severity: 'medium', message: `Quotation "${row.opportunity.title}" menunggu konfirmasi Anda`, relatedRoute: `/client/opportunities/${row.opportunity.id}` })
  }
  if (clientScopeId.value) {
    for (const approval of getPendingClientApprovals(clientScopeId.value)) {
      items.push({ id: `approval-${approval.id}`, severity: 'medium', projectId: approval.projectId, message: `Approval "${approval.entityType}" pada project ${approval.projectId} menunggu keputusan Anda`, relatedRoute: '/client/approvals' })
    }
  }
  ...
  for (const ticket of openSupportTickets.value) {
    items.push({ id: `support-${ticket.id}`, severity: 'low', message: `Support ticket "${ticket.subject}" masih ${ticket.status}`, relatedRoute: '/client/support' })
  }
  return items
})
...
const reviewApprovalTarget = computed(() => quotationsAwaitingConfirm.value[0]
  ? `/client/opportunities/${quotationsAwaitingConfirm.value[0].opportunity.id}`
  : '/client/approvals')
...
const quickActions = computed(() => [
  ...
  { key: 'trip-center', label: 'Open Trip Center', icon: Compass, to: '/client/trip-center' },
  { key: 'payment', label: 'Upload Payment Proof', icon: Upload, to: '/client/billing' },
  { key: 'support', label: 'Create Support Ticket', icon: LifeBuoy, to: '/client/support' }
])
