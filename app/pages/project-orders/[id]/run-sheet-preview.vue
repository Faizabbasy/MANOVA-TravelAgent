...
import {
  getProjectById, getUserById, getVendorById, getVendorContacts,
  getItineraryItems, getProjectServices, getTravelers
} from '~/data'
import { formatDateRange, formatDayLabel, maskDocumentNumber } from '~/utils/format'
    ...
    <div class="mx-auto max-w-4xl px-4 print:px-0 print:max-w-none">
      ...
      <template v-else>
        <div class="mb-4 flex items-center justify-between print:hidden">
          <NuxtLink :to="`/project-orders/${project.id}?tab=itinerary-services`" class="text-sm text-primary hover:underline">
            ← Kembali ke Itinerary & Services
          </NuxtLink>
        ...
        <div class="rounded-xl border border-border bg-card p-8 shadow-sm print:rounded-none print:border-0 print:shadow-none print:p-0">
          <div class="flex items-start justify-between border-b border-border pb-6 mb-6">
            <div>
              ...
              <p class="text-xs text-muted-foreground">
                Travel Agent B2B — Operational Run Sheet (Mock)
              </p>
            </div>
            <div class="text-right">
              ...
              <p class="text-xs text-muted-foreground">
                {{ formatDateRange(project.travelStartDate, project.travelEndDate) }} · {{ project.destination }}
              </p>
            </div>
          </div>
                ...
                <table class="w-full text-sm mb-2">
                  <tbody>
                    <tr v-for="item in day.items" :key="item.id" class="border-b border-border/60">
                      <td class="py-1.5 w-32 text-muted-foreground">
                        {{ item.time ?? '—' }}<template v-if="item.timezone">
                          ({{ item.timezone }})
                        </template>
                      </td>
                      <td class="py-1.5 text-foreground">
                        {{ item.title }}<span v-if="item.visibleToClient === false" class="text-xs text-muted-foreground"> (Internal Only)</span>
                      </td>
                    </tr>
                  </tbody>
                </table>
              ...
              <tbody>
                ...
                <tr v-if="team.length === 0">
                  <td colspan="3" class="py-2 text-muted-foreground">
                    Belum ada anggota tim ditugaskan.
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div class="mb-6">
            ...
            <table class="w-full text-sm">
              ...
              <tbody>
                ...
                <tr v-if="vendorContacts.length === 0">
                  <td colspan="3" class="py-2 text-muted-foreground">
                    Belum ada vendor ditugaskan.
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div>
            <p class="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-2">
              Emergency Info Traveler
            </p>
            <p v-if="!canViewFullContacts" class="text-xs text-muted-foreground mb-2">
            ...
            <table class="w-full text-sm">
              ...
              <tbody>
                ...
                <tr v-if="emergencyContacts.length === 0">
                  <td colspan="3" class="py-2 text-muted-foreground">
                    Belum ada kontak darurat tercatat.
                  </td>
                </tr>
              </tbody>
            </table>
