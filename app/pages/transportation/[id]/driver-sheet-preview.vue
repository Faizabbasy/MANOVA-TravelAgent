        ...
        <div class="rounded-xl border border-border bg-card p-8 shadow-sm print:rounded-none print:border-0 print:shadow-none print:p-0">
          <div class="flex items-start justify-between border-b border-border pb-6 mb-6">
            <div>
              ...
              <p class="text-xs text-muted-foreground">Internal — Driver Sheet (Mock, tanpa informasi harga)</p>
            </div>
            <div class="text-right">
              <p class="text-lg font-semibold text-foreground">DRIVER SHEET</p>
          ...
          <div class="grid grid-cols-2 gap-6 mb-6 text-sm">
            <div>
              <p class="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-1">Driver / Unit</p>
              <p class="text-foreground">{{ booking.driverName ?? 'Belum ditugaskan' }}<span v-if="booking.driverPhone"> · {{ booking.driverPhone }}</span></p>
              <p class="text-muted-foreground">Plat: {{ booking.assignedVehiclePlateNumber ?? '—' }}</p>
            </div>
            <div>
              <p class="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-1">Kendaraan</p>
              <p v-if="selectedOption" class="text-foreground">{{ findStatusOption(VEHICLE_TYPES, selectedOption.vehicleType).label }} — kapasitas {{ selectedOption.capacity }} pax</p>
          ...
          <div v-if="booking.standbyHours || booking.overtimeHours || booking.tollFeeIdr" class="mb-6 text-sm">
            <p class="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-2">Catatan Operasional</p>
            <ul class="text-muted-foreground list-disc list-inside space-y-1">
              <li v-if="booking.standbyHours">Standby {{ booking.standbyHours }} jam</li>
              <li v-if="booking.overtimeHours">Overtime {{ booking.overtimeHours }} jam</li>
              <li v-if="booking.tollFeeIdr">Estimasi toll dibebankan ke perusahaan — simpan struk toll</li>
            </ul>
          </div>

          <div class="mb-6">
            <p class="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-2">Passenger Manifest ({{ passengers.length }} pax)</p>
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-border text-left text-[11px] uppercase tracking-wide text-muted-foreground">
                  <th class="py-2">Nama</th>
                  <th class="py-2">Kontak Darurat</th>
                  <th class="py-2">Catatan Khusus</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="passenger in passengers" :key="passenger.id" class="border-b border-border/60">
                  <td class="py-2 text-foreground">{{ passenger.name }}</td>
                  <td class="py-2 text-muted-foreground">{{ passenger.emergencyContactName ? `${passenger.emergencyContactName} (${passenger.emergencyContactPhone ?? '—'})` : '—' }}</td>
                  <td class="py-2 text-muted-foreground">{{ [passenger.specialRequest, passenger.dietaryRestrictions, passenger.accessibilityNeeds].filter(Boolean).join(' · ') || '—' }}</td>
                </tr>
                <tr v-if="passengers.length === 0"><td colspan="3" class="py-2 text-muted-foreground">Belum ada traveler ditugaskan.</td></tr>
              </tbody>
            </table>
          </div>

          <div v-if="booking.hasChange || booking.hasIncident" class="border-t border-border pt-4 text-xs text-muted-foreground space-y-2">
            <p v-if="booking.hasChange"><span class="font-semibold text-foreground">Change:</span> {{ booking.changeNote }}</p>
            <p v-if="booking.hasIncident"><span class="font-semibold text-foreground">Incident:</span> {{ booking.incidentNote }}</p>
          </div>
