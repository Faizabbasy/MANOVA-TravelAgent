...
**Catatan implementasi Prompt 20 (Change Request, supersede sebagian — khusus eksekusi Won, D-053):** Model dua-langkah "Submit as Won" (AE) → "Approve Won" (Management) terpisah **tidak lagi dipakai di UI**. Setelah Commercial Approval (lapisan Prompt 19 di atas) disetujui Management, **Account Executive langsung "Mark as Won"** dalam satu aksi — Management tetap satu-satunya approver, tapi approval-nya terjadi di titik Commercial Approval (Quotation), bukan lagi di titik eksekusi Won terpisah. Secara teknis, transisi internal `negotiation → won-requested → won` tetap terjadi (reuse mutator existing, dipanggil berurutan dalam satu klik) — union `OpportunityStage` **tidak diubah**, hanya urutan pemanggilan dari UI yang berbeda. Lihat `docs/mockup-design-decisions.md` D-053 untuk rasional lengkap dan `docs/mockup-change-impact-log.md` CI-027.
