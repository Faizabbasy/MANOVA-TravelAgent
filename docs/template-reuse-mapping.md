# Template Reuse Mapping — MANOVA (Prompt 2, dilengkapi di Prompt 4, dieksekusi sebagian di Prompt 5)
...
| Sidebar item `Time Tracking` | `/time-tracking` (dead link) | Entri menu saja | — | `REMOVE_AFTER_VALIDATION` | — | **Ya** | Hanya `AppSidebar.vue menuItems`, tidak ada page/store/middleware | Rendah (aman dihapus) | **DONE (Prompt 5)** — menu lama diganti total oleh `NAV_ITEMS` baru, item ini otomatis tidak ada lagi |
| Sidebar item `Integrations` | `/integrations` (dead link) | Entri menu saja | — | `REMOVE_AFTER_VALIDATION` | — | **Ya** | Hanya entri menu | Rendah (aman dihapus) | **DONE (Prompt 5)** — sama seperti di atas |
| Sidebar item `Templates` | `/templates` (dead link) | Entri menu saja | — | `REMOVE_AFTER_VALIDATION` (butuh validasi) | — | **Ya, menunggu validasi** (Q di `docs/mockup-open-questions.md` arsip) | Hanya entri menu | Rendah | **DONE (Prompt 5)** — tidak ada di `NAV_ITEMS` baru; validasi eksplisit tidak diperlukan lagi karena item sudah tidak ada, bukan karena divalidasi terpisah |
| Sidebar item `Files` | `/files` (dead link) | Entri menu saja | Tab "Documents" Project Detail | `REUSE_LAYOUT_REPLACE_CONTENT` (sebagai tab, bukan top-level) | Nama/ikon dipakai ulang untuk label tab | **Ya, sebagai menu top-level** (bukan konsepnya) | Hanya entri menu | Rendah | **DONE (shell, Prompt 5)** — tab "Documents" di Project Detail sudah menampilkan data fixture nyata; menu top-level lama sudah tidak ada |
| `dashboard/AIAssistant.vue` | Dirender di `/` | Komponen statis tanpa data model | — (tidak dilanjutkan, D-023) | `KEEP_TEMPORARILY` → **excluded dari desain baru** | — | **Ya — DIEKSEKUSI di Prompt 5** | Dirender di `pages/index.vue` lama, tidak ada store/middleware lain — dependency check ulang mengonfirmasi nol referensi sebelum dihapus | Rendah | **DONE (Prompt 5)** — file dihapus, lihat D-044 |
| `app/lib/utils.ts` vs `app/utils/cn.ts` | — (utility, bukan route) | `cn()` duplikat persis | Satu sumber `cn()` | Konsolidasi teknis — **DIEKSEKUSI (bukan lagi DEFERRED)** | Pilih satu, update seluruh pemanggil | **Ya — DIEKSEKUSI di Prompt 5** | Dependency check (`grep` seluruh `app/`) mengonfirmasi seluruh pemanggil `cn()` sudah eksplisit import dari `~/lib/utils`, nol pemakaian `app/utils/cn.ts` | Rendah (terverifikasi sebelum dihapus, bukan "sedang" lagi) | **DONE (Prompt 5)** — `app/utils/cn.ts` dihapus, lihat D-044 |
| `dialog/DialogContent.vue` vs `DialogScrollContent.vue` | — (komponen, bukan route) | Dua Dialog ~90% identik | Satu komponen dengan prop `scrollable` | Konsolidasi teknis (DEFERRED, D-017) | Gabung jadi satu dengan prop | Tidak (keduanya dipakai aktif, bukan dead code) | Dipakai di banyak modal existing | Rendah | Belum dikerjakan — tetap DEFERRED, tidak disentuh di Prompt 5 |
| `.gradient-primary` CSS class | — (utility CSS) | Class tidak dipakai di manapun | — | `REMOVE_AFTER_VALIDATION` | — | **Ya — DIEKSEKUSI di Prompt 5** | Dependency check ulang (`grep` lintas `app/`+`assets/`) mengonfirmasi nol elemen yang mereferensikan | Rendah | **DONE (Prompt 5)** — dihapus dari `assets/css/tailwind.css` |
| `dashboard/BudgetChart.vue`, `ExpenseCategories.vue`, `ProjectsTable.vue`, `RecentActivity.vue`, `TasksOverview.vue`, `TeamMetrics.vue` | Dirender di `/` (lama) | Komponen dashboard existing | Basis Finance/Project/Reporting widget (lihat bagian D) | `REUSE_COMPONENTS`/`ADAPT` | Refactor jadi props-driven per domain masing-masing | **Tidak** — bukan `REMOVE_AFTER_VALIDATION`, sengaja dipertahankan meski kini tidak dirender di manapun | Tidak dirender lagi sejak `pages/index.vue` ditulis ulang (Prompt 5), tapi tidak ada file lain yang bergantung | Rendah | Menunggu fase Finance/Project Management/Reporting untuk diadaptasi — **bukan dihapus**, sesuai instruksi "jangan menghapus file hanya berdasarkan nama" |
